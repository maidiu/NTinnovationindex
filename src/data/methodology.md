# Methodology: NT Continuity Index

## Overview

The NT Continuity Index is a computational instrument that quantifies lexical, semantic, and conceptual departure from a Hebrew-rooted Greek baseline across all 27 New Testament books. The instrument treats the Masoretic Text (MT) as the anchor of the Hebrew semantic world, uses the Septuagint (LXX) as the Greek measurement baseline mediating that world, and scores each NT book along six weighted axes that compose a single Departure score on a 0–100 scale — which is then displayed as its inverse (Continuity = 100 − Departure) so that higher values indicate greater fidelity to the inherited tradition. The project aims for academic objectivity: given the same data and scoring rubric, researchers with different theological commitments should arrive at identical or near-identical measurements.
